                    ...
                    try {
                        ...
                        while (addresses.hasMoreElements()) {
                            try {
                                ...
                                logger.warn("Failed to get host address. cause: {}", e.getMessage());
                            }
                        }
                    } catch (Throwable e) {
                        logger.warn("Failed to get network address. cause: {}", e.getMessage());
                    }
                }
            }
        } catch (Throwable e) {
            logger.warn("Failed to get network interface. cause: {}", e.getMessage());
        }

        logger.error("Could not get host ip address, will use 127.0.0.1 instead.");
        return localAddress;
    }
