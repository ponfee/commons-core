...
public class DAGExpressionParserTest {
    ...
    public void testProcess() {
        Assert.assertEquals("((A)->(((B)->(C)->(D)),((A)->(F)))->((G),(H),(X))->(J))", DAGExpressionParser.completeParenthesis("(A->((B->C->D),(A->F))->(G,H,X)->J)"));
        Assert.assertEquals("(A),(B)->((C)->(D)),(E)->(F)", DAGExpressionParser.completeParenthesis("A,B->(C->D),(E)->F"));
        Assert.assertEquals("(A),(B)->((C)->(D)),(E)->(F)", DAGExpressionParser.completeParenthesis("A,B->(C->D),E->F"));
    }
    ...
    public void testGraph() {
        ...
        Assert.assertEquals("[1:1:A, 2:3:A]", graph.successors(GraphNodeId.START).toString());
        Assert.assertTrue(graph.predecessors(GraphNodeId.START).isEmpty());

        Assert.assertEquals("[1:1:J, 2:1:Y]", graph.predecessors(GraphNodeId.END).toString());
        Assert.assertTrue(graph.successors(GraphNodeId.END).isEmpty());
    ...
    public void testGraphNodeId() {
        Assert.assertTrue(GraphNodeId.fromString(GraphNodeId.START.toString()) == GraphNodeId.START);
        Assert.assertTrue(GraphNodeId.fromString(GraphNodeId.END.toString()) == GraphNodeId.END);
        Assert.assertEquals(GraphNodeId.fromString("1:1:test").toString(), "1:1:test");
        Assert.assertEquals(GraphNodeId.fromString("1:1:test:ANY").getName(), "test:ANY");
        Assert.assertEquals(GraphNodeId.fromString("1:1:test:ALL").getName(), "test:ALL");
        Assert.assertEquals(GraphNodeId.fromString("1:1:test:ALL").toString(), "1:1:test:ALL");
    }
